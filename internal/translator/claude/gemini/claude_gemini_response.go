...
func ConvertClaudeResponseToGemini(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) ([]string, error) {
	if *param == nil {
	...
	case "error":
		...
		template, _ = sjson.Set(template, "error.message", errorMsg)
		template, _ = sjson.Set(template, "error.code", 500)
		return []string{template}, nil
	}
...
// ConvertClaudeResponseToGeminiNonStream converts a non-streaming Claude Code response to Gemini format.
// This function processes the complete Claude Code API response and transforms it into a single
// Gemini-compatible JSON response. It handles all content types including text, thinking content,
// and tool calls, consolidating them into the appropriate Gemini response structure.
//
// Parameters:
//   - ctx: The context for the request
//   - modelName: The name of the model being used
//   - rawJSON: The raw JSON response from the Claude Code API (may contain multiple SSE events)
//   - param: A pointer to a parameter object (unused for non-streaming)
//
// Returns:
//   - string: A Gemini-compatible JSON response string
func ConvertClaudeResponseToGeminiNonStream(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) (string, error) {
	...
	// If no SSE events found, try parsing as direct JSON
	if len(streamingEvents) == 0 {
		streamingEvents = append(streamingEvents, rawJSON)
	}

	// Track state across events using the same param struct as streaming
